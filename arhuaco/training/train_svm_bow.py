...
class TrainSvmBow:
    ...
    def train(self,
        ...
        # Buid the model
        svm_bow.build_model(learn_rate=configuration['learn_rate'],
        ...
        training_generator = data_helper.get_data_BoW_chunk(vocabulary,
                                                   configuration['labels_svm'])
        validation_generator = data_helper.get_data_BoW_chunk(vocabulary,
                                                configuration['labels_svm'])
        ...
        test_generator = validation_data_helper.get_data_BoW_chunk(vocabulary,
                                                configuration['labels_svm'])

        result = svm_bow.test_model(test_data_source=test_generator,
...
def main(argv):
    try:
       ...
       print("Usage: test_svm_bow.py -t <type>")
       sys.exit(2)
    for opt, arg in opts:
       if opt == '-h':
          print("Usage: test_svm_bow.py -t <type>")
    ...
    from arhuaco.training.train_svm_bow import TrainSvmBow
    training = TrainSvmBow()
    if type == "syscall":
        training.train("syscall")
    elif type == "network":
        training.train("network")
    else:
        print("Usage train_svm_bow.py -t <type>")
